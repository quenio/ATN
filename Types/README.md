...
Note that, in the example above, the intersection of the constituent types is an empty set. That may *not* be the case with a type union, sometimes. In example below, the intersection of `IntegerNumber` and `RationalNumber` is not an empty set:
