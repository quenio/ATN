...
Note that, in the example above, the intersection of the constituent types is an empty set. Sometimes, that may *not* be the case with a type union. In example below, the intersection of `IntegerNumber` and `RationalNumber` is not an empty set:
