...
Note that, in the example above, the intersection of the constituent type is an empty set. That may not always be the case with a type union. In example below, the intersection of `IntegerNumber` and `RationalNumber` is not an empty set:
